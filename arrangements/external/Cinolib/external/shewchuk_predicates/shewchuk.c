...
//////////////////////// Orient3D with minor chaching

// Fill 'minor' with three 2x2 determinants representing the minors for triangle <pb,pc,pd>
// 'perm' will be filled with information to derive error bounds.
// It is responsibility of the caller to pre-allocate space for three REALs in both minor and perm.

void orient3d_get_minors(REAL* pb, REAL* pc, REAL* pd, REAL* minor, REAL* perm)
{
    const REAL bdx = pb[0] - pd[0];
    const REAL cdx = pc[0] - pd[0];
    const REAL bdy = pb[1] - pd[1];
    const REAL cdy = pc[1] - pd[1];
    const REAL bdz = pb[2] - pd[2];
    const REAL cdz = pc[2] - pd[2];

    const REAL bdycdz = bdy * cdz;
    const REAL cdybdz = cdy * bdz;
    minor[0] = bdycdz - cdybdz;
    perm[0] = Absolute(bdycdz) + Absolute(cdybdz);

    const REAL bdxcdz = bdx * cdz;
    const REAL cdxbdz = cdx * bdz;
    minor[1] = cdxbdz - bdxcdz;
    perm[1] = Absolute(cdxbdz) + Absolute(bdxcdz);

    const REAL bdxcdy = bdx * cdy;
    const REAL cdxbdy = cdx * bdy;
    minor[2] = bdxcdy - cdxbdy;
    perm[2] = Absolute(bdxcdy) + Absolute(cdxbdy);
}


// Calculate orient3d(pa,pb,pc,pd) based on pre-calculated minors and error permanent factors

REAL orient3d_with_cached_minors(REAL* pa, REAL* pb, REAL* pc, REAL* pd, REAL* minor, REAL* perm)
{
    const REAL adx = pa[0] - pd[0];
    const REAL ady = pa[1] - pd[1];
    const REAL adz = pa[2] - pd[2];

    const REAL det = adx * minor[0] + ady * minor[1] + adz * minor[2];

    const REAL permanent = Absolute(adx) * perm[0] + Absolute(ady) * perm[1] + Absolute(adz) * perm[2];

    const REAL errbound = o3derrboundA * permanent;
    if ((det > errbound) || (-det > errbound)) {
        return det;
    }
...
// Example re-implementation of the 'standard' orient3d predicate with minor caching
/*
REAL orient3d(REAL* pa, REAL* pb, REAL* pc, REAL* pd)
{
    REAL minor[3], perm[3];
    orient3d_get_minors(pa, pb, pc, pd, minor, perm);
